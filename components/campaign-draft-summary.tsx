...
export async function CampaignDraftSummary({ campaignId, recipientCount, isDraft }: {
  ...
  const statuses = ["generated", "edited", "approved", "excluded", "failed", "processing"] as const;
  ...
  const successful = counts.generated + counts.edited + counts.approved + counts.excluded;
  ...
  return <section className="mt-10">
    ...
    <div className="card mt-6 flex flex-wrap items-center justify-between gap-5 rounded-2xl p-6"><div><h2 className="text-xl font-bold tracking-[-.03em]">Draft review</h2><p className="mt-1 text-sm text-[#65736f]">{counts.approved} approved · {counts.edited} edited · {counts.failed} failed · {Math.max(0, recipientCount - counts.approved - counts.excluded)} need attention</p>{hasError ? <p role="alert" className="mt-2 text-sm text-[#b42318]">Could not load all draft totals.</p> : null}</div><Link className="button-primary" href={`/campaigns/${campaignId}/drafts`}>Review Drafts <ArrowRight size={17} /></Link></div>
  </section>;
}
