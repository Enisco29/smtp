...
export default async function DraftPage({ params }: { params: Promise<{ id: string; draftId: string }> }) {
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <div className="mt-7 grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
      <div>{draft.subject && draft.body ? <DraftReviewControls campaignId={id} draftId={draft.id} subject={draft.subject} body={draft.body} status={draft.status} revision={draft.content_revision} /> : <div className="card rounded-2xl p-6"><h2 className="font-bold">Draft unavailable</h2><p className="mt-2 text-sm text-[#65736f]">This draft is {draft.status}. Return to the campaign to retry generation.</p>{draft.failure_code ? <p className="mt-2 text-sm text-[#b42318]">{draft.failure_code.replaceAll("_", " ")}</p> : null}</div>}</div>
